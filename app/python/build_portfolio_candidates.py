...
ANTLER_PAGE_1 = SourcePage(
    source="antler",
    url="https://www.antler.co/portfolio",
    cache_name="antler_portfolio.html",
)
B2VENTURE_PAGE_1 = SourcePage(
    source="b2venture",
    url="https://www.b2venture.vc/portfolio",
    cache_name="b2venture_portfolio.html",
)
...
def parse_antler(max_pages: int, refresh: bool) -> list[dict]:
    pages = iter_paginated_pages(ANTLER_PAGE_1, "0b933bfd_page", max_pages, refresh)
    records: list[dict] = []
    seen_urls: set[str] = set()
    splitter = '<div role="listitem" class="w-dyn-item"><div class="portco_card">'

    for page_url, body in pages:
        section_match = re.search(
            r'<div[^>]+class="portco_cms_list[^"]*"[^>]*>(.*?)<div role="navigation" aria-label="List" class="w-pagination-wrapper portco_cms_pagination">',
            body,
            flags=re.S,
        )
        ...
        for block in section.split(splitter)[1:]:
            href_match = re.search(r'<a target="_blank" href="([^"]+)" class="clickable_link', block)
            if not href_match:
                continue
            ...
            name = re.search(
                r'fs-cmsfilter-field="name"[^>]*class="u-text[^"]*"[^>]*>([^<]+)</p>',
                block,
            )
            description = re.search(
                r'fs-cmsfilter-field="description"[^>]*class="u-text[^"]*"[^>]*>(.*?)</p>',
                block,
                flags=re.S,
            )
            tag_values = re.findall(r'<div fs-cmsfilter-field="location" class="tag_small_text">([^<]+)</div>', block)
            ...
            record = make_record(
                ...
                portfolio_url=page_url,
                description=description.group(1) if description else None,
                tags=[tag_values[1]] if len(tag_values) >= 2 else [],
                country=tag_values[0] if len(tag_values) >= 1 else None,
                investment_year=tag_values[2] if len(tag_values) >= 3 else None,
            )
            if record:
                records.append(record)
...
def parse_b2venture(max_pages: int, refresh: bool) -> list[dict]:
    pages = iter_paginated_pages(B2VENTURE_PAGE_1, "aa668c5b_page", max_pages, refresh)
    records: list[dict] = []
    seen_urls: set[str] = set()
    splitter = '<div role="listitem" class="company-item w-dyn-item">'

    for page_url, body in pages:
        for block in body.split(splitter)[1:]:
            href_match = re.search(r'<a href="(https?://[^"]+)" target="_blank" class="portfolio-table-grid link w-inline-block">', block)
            if not href_match:
                continue
            ...
            name = re.search(r'class="headline-05 weight--semibold">([^<]+)</div>', block)
            description = re.search(r'class="portfolio-description">(.+?)</div>', block, flags=re.S)
            country = re.search(r'fs-cmssort-field="region"[^>]*class="headline-05">([^<]+)</div>', block)
            investment_year = re.search(r'fs-cmssort-field="year"[^>]*class="headline-05">(\d{4})</div>', block)
            tags = re.findall(r'fs-cmsfilter-field="sector" class="label-01">([^<]+)</div>', block)
            ...
            record = make_record(
                ...
                investment_year=investment_year.group(1) if investment_year else None,
            )
            if record:
                records.append(record)
...
def main() -> int:
    ...
    parser.add_argument("--antler-pages", type=int, default=25, help="Max pages to fetch for Antler")
    parser.add_argument("--b2venture-pages", type=int, default=25, help="Max pages to fetch for b2venture")
    ...
    source_builders = [
        ...
        ("antler", lambda: parse_antler(args.antler_pages, args.refresh)),
        ("b2venture", lambda: parse_b2venture(args.b2venture_pages, args.refresh)),
